...
class LLM2Vec(nn.Module):
    ...
    def prepare_for_tokenization(self, text):
        def _is_instruct(name):
            return ("chat" in name.lower()) or ("instruct" in name.lower()) or ("sharegpt" in name.lower())
        
        if _is_instruct(self.model.config._name_or_path):
            text = '[INST] ' + text.strip() + ' [/INST]'
        if (isinstance(self.model.config, LlamaConfig) or isinstance(self.model.config, MistralConfig)) and self.pooling_mode == "eos_token":
            text = text.strip() + ' </s>'
        return text
    ...
    def _convert_to_str(self, instruction, text):   
        tokenized_q = self.tokenizer(text, return_tensors='pt', padding=True, truncation=True, max_length=self.max_length, add_special_tokens=False)
